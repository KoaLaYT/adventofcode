 ...
 *   '[^abc]'   Inverted class, match if NOT one of {'a', 'b', 'c'} -- NOTE:
...
{
    /* The sizes of the two static arrays below substantiates the static RAM
       usage of this module. MAX_REGEXP_OBJECTS is the max number of symbols in
       the expression. MAX_CHAR_CLASS_LEN determines the size of buffer for
       chars in all char-classes in the expression. */
    ...
    while (pattern[i] != '\0' && (j + 1 < MAX_REGEXP_OBJECTS)) {
        ...
        switch (c) {
            ...
            case '?': {
                ...
                /*    case '|': {    re_compiled[j].type = BRANCH;          }
            ...
            case '[': {
                ...
                if (pattern[i + 1] == '^') {
                    ...
                    i += 1; /* Increment i to avoid including '^' in the
                               char-buffer */
                    if (pattern[i + 1] == 0) /* incomplete pattern, missing
                                                non-zero char after '^' */
                    {
                        return 0;
                    }
...
{
    const char* types[] = {
        "UNUSED",         "DOT",       "BEGIN", "END",        "QUESTIONMARK",
...
static int matchpattern(regex_t* pattern, const char* text, int* matchlength)
{
    ...
    do {
        ...
        /*  Branching is not working properly
            ...
            {
              return (matchpattern(pattern, text) || matchpattern(&pattern[2],
           text));
            }
        */
        (*matchlength)++;
